 ...
 * @since 1.4
 */
public class PieMenuPopup extends Popup {
	 ...
	 *                if contents is null
	 */
	protected PieMenuPopup(Component owner, Component contents, int x, int y) {
		this();
		if (contents == null) {
	...
	void dispose() {
		Component component1 = getComponent();
		final Window window = SwingUtilities.getWindowAncestor(component1);
	 ...
	 */
	void reset(final Component owner, final Component contents,
			final int ownerX, final int ownerY) {
		if (getComponent() == null) {
			this.component = createComponent(owner);
		}

		Component c = getComponent();

		if (c instanceof JWindow) {
			JWindow component1 = (JWindow) getComponent();

			component1.setLocation(ownerX - c.getWidth() / 2, ownerY
					- c.getHeight() / 2);
			component1.getContentPane().add(contents, BorderLayout.CENTER);
			contents.invalidate();
	 ...
	 */
	void pack() {
		final Component component1 = getComponent();
	...
	// implements ModalExclude
	{
		 ...
		 * {@link long} COMMENT.
		 */
		private static final long serialVersionUID = 5928649383160876529L;

		final JPieMenue innerPie = new JPieMenue();
		...
		HeavyWeightWindow(Window parent) {
			...
			this.innerPie.setBorder(BorderFactory
					.createLineBorder(Color.LIGHT_GRAY));
			getRootPane().setContentPane(this.innerPie);
			setSize(this.innerPie.getSize());
			this.innerPie.setLocation(0, 0);
		}
	...
	static class DefaultFrame extends Frame {
		 ...
		 * {@link long} COMMENT.
		 */
		private static final long serialVersionUID = 8781511108190374096L;
	}
}
